...
/**
 * Dietary Filter enum for recipe categories
 */
...
/**
 * Cuisine Type enum for recipe categories
 */
 ...
 * These are based on common recipe categories that work well with Spoonacular API
 */
object RecipeCategorySamples {
    fun getRecipeCategories(): List<RecipeCategory> = listOf(
     ...
     * Get category by ID
     */
    fun getCategoryById(categoryId: String): RecipeCategory? {
        return getRecipeCategories().find { it.id == categoryId }
    }
    
    /**
     * Search categories by name
     */
    fun searchCategories(query: String): List<RecipeCategory> {
        return getRecipeCategories().filter { 
    ...
    /**
     * Filter categories by dietary preference
     */
    fun filterByDietaryPreference(filter: DietaryFilter): List<RecipeCategory> {
        if (filter == DietaryFilter.ALL) {
            return getRecipeCategories()
        }
        return getRecipeCategories().filter { category ->
            category.dietaryTags.contains(filter)
        }
    }
    
    /**
     * Filter categories by cuisine type
     */
    fun filterByCuisineType(cuisine: CuisineType): List<RecipeCategory> {
        return getRecipeCategories().filter { it.cuisineType == cuisine }
    }
    
    /**
     * Get categories with combined filters
     */
    fun getFilteredCategories(
        dietaryFilter: DietaryFilter = DietaryFilter.ALL,
        cuisineFilter: CuisineType? = null,
        searchQuery: String = ""
    ): List<RecipeCategory> {
        var filteredCategories = getRecipeCategories()
        
        // Apply dietary filter
        if (dietaryFilter != DietaryFilter.ALL) {
            filteredCategories = filteredCategories.filter { category ->
                category.dietaryTags.contains(dietaryFilter)
            }
        }
        
        // Apply cuisine filter
        cuisineFilter?.let { cuisine ->
            filteredCategories = filteredCategories.filter { it.cuisineType == cuisine }
        }
